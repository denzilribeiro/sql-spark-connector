...
### When using savemod `overwrite` if you do not use the option `truncate` , on recreation of the table you will lose indexes. For example a columnstore table would now be a heap. If you want to maintain existing indexing please also specify option `truncate` with value true. i.e .option("truncate",true)
```python
